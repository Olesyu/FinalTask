...
public class ColorUtils {
     ...
     * @return
     */
    public String getColorNameFromRGBCSSString(String input) {
        String in = input.toLowerCase();
        if (!in.startsWith("rgb(")) {
            return "Error!";
        }
        String[] num = in.replaceAll("[^0-9,]", "").split(",");
        int[] rgbValues = Arrays.stream(num).mapToInt(Integer::parseInt).toArray();
        if(rgbValues.length != 3) {
            return "Error!";
        }
        return this.getColorNameFromRgb(rgbValues[0], rgbValues[1], rgbValues[2]);
    }
